...
class Evaluation:
    ...
    _EVAL_CACHE_MAX = 200_000  # 评估缓存最大容量，超出时整体清空以防内存膨胀
    ...
    # ════════════════════════════════════════════════════════════
    #  3. Tapered Evaluation 阶段因子与子力基础价值
    # ════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════
    #  4. 位置分值表（PST）——全部以红方视角编写，黑方使用时翻转行号
    # ════════════════════════════════════════════════════════════
    ...
    # ── 马的中局位置分值表 ──
    # 行 0–2、列 3/6 分值最高（18–22），对应"挂角马"与"卧槽马"经典站位。
    # 挂角马从对方九宫斜角直接威胁将帅，卧槽马卡在九宫底角形成难以解除的攻势。
    # 中局阶段子力密集，马到达这些高价值格的威胁远大于残局，
    # 因此 MG 表的峰值高于 EG 表。
    PST_MA_MG = [
        [0, 1, 4, 18, 6, 18, 4, 1, 0],
    ...
    # ── 棋子机动性奖励 ──
    _MA_MOBILITY_STEP = 5.0      # 马的每个可达空格/敌方格的机动性奖励
    _MA_DOUBLE_SYNERGY = 15.0    # 连环马协同奖励（落点为己方马时，两马互保价值高）
    _PAO_SCREEN_PER_DIR = 18.0   # 炮的每个方向发现己方炮架时的奖励
    ...
    _PENALTY_MISSING_XIANG = 30.0
    # 缺士面对双车时极度危险（双车错杀、铁门栓等杀法对缺士方几乎无解）
    _PENALTY_MISSING_SHI = 50.0
    ...
    def _ma_mobility(board: Board, sr: int, sc: int, color: str) -> float:
        ...
        功能说明：
            ...
            马腿畅通的每个落点：
            - 若落点为空或对方棋子，加 _MA_MOBILITY_STEP 分（可用机动步）。
            - 若落点为己方的马，加 _MA_DOUBLE_SYNERGY 分（连环马协同奖励——两马
              相互保护，战术价值高于单马）。
        ...
        for dr, dc in Evaluation._MA_DELTAS:
            ...
            if tgt is not None and tgt.color == color:
                ...
                if tgt.piece_type == "ma":
                    bonus += Evaluation._MA_DOUBLE_SYNERGY
                continue
            # 落点为空或对方棋子 → 可用机动步
            bonus += Evaluation._MA_MOBILITY_STEP
        return bonus
    ...
    def _pao_screen_bonus(board: Board, r: int, c: int, color: str) -> float:
        ...
        功能说明：
            ...
            则视为潜在炮架，给予 _PAO_SCREEN_PER_DIR 分奖励。
        ...
        for dr, dc in Evaluation._ORTH:
            ...
            while 0 <= tr < 10 and 0 <= tc < 9:
                ...
                # 遇到的第一枚棋子为己方 → 可作为炮架
                if p.color == color:
                    bonus += Evaluation._PAO_SCREEN_PER_DIR
                break
        return bonus
    ...
    def _tactical_synergy(
        ...
        if is_ma_deep and is_che_on_flank:
    ...
    def evaluate(board: Board) -> float:
        ...
        功能说明：
            ...
            5. **阵型惩罚/奖励**：
               - 缺象 + 对方有炮 → 对方加分（炮打空头效率高）。
               - 缺士 + 对方双车 → 对方加分（双车杀缺士效率极高）。
        ...
        # ── 步骤 5：阵型惩罚/奖励 ──
        penalty_xiang = Evaluation._PENALTY_MISSING_XIANG
        penalty_shi = Evaluation._PENALTY_MISSING_SHI
        if red_xiang < 2 and black_pao > 0:
            black_mg += penalty_xiang
            black_eg += penalty_xiang
        if black_xiang < 2 and red_pao > 0:
            red_mg += penalty_xiang
            red_eg += penalty_xiang
        if red_shi < 2 and black_che == 2:
            black_mg += penalty_shi
            black_eg += penalty_shi
        ...
        # ── 步骤 10：缓存写入（超过限额时清空，防止内存膨胀） ──
        if len(Evaluation._eval_cache) > Evaluation._EVAL_CACHE_MAX:
