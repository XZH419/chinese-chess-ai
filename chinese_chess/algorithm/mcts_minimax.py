...
_PROBE_PROB_LIGHT = 0.48   # Level 1 触发概率（略增 → 战术局面更多轻量 probe）
_PROBE_PROB_FULL = 0.88    # Level 2 触发概率

# ── 冷却 ──
_PROBE_COOLDOWN_SIMS = 5   # 两次 probe 之间最少间隔模拟数

# ── Policy：probe 触发 / 排序 轻量进攻偏置（不加到 Evaluation 分值上）──
_SCORE_POLICY_PRESSURE = 3   # 存在将军着法或高价值吃子机会时加分（略增 → 更易触发 probe）
_PROBE_ATTACK_BONUS_CHECK = 170
_PROBE_ATTACK_BONUS_AGGRESSIVE = 72
