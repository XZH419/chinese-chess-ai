...
Physical migration from `chinese-chess/ai/ai_mcts.py` with only import-path
...
class MCTSNode:
    def __init__(self, board, parent=None, move=None):
        ...
        self.value = 0
        # 当前状态下尚未扩展的走法列表。
        self.untried_moves = Rules.get_all_moves(board, board.current_player)
...
class MCTSAI:
    ...
    def simulate(self, board):
        ...
        while not Rules.is_game_over(current_board):
            moves = Rules.get_all_moves(current_board, current_board.current_player)
            if not moves:
                break
